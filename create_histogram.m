...
while (nm<numberfiles-0)
    ...
    [n_f,~]=histogram(fret,gap);          
    [n_fs,~]=histogram(fret_smooth,gap);  
    [n_F,~]=histogram(FRET_filter,gap);  
    [n_F_gamma,~]=histogram(FRET_filter_gamma,gap);  
    [n_fret,x]=histogram(fret_filter,gap);  
